...
func request(client *http.Client, method, endpoint string, payload []byte) (int, []byte, error) {
	...
	if len(data) > 1<<20 {
		return 0, nil, errors.New("bounded response exceeded")
	}
...
func run() error {
	...
	forkVersion := flag.String("fork-version", "", "exact intended native fork version")
	expectedKeyHash := flag.String("public-key-sha256", "", "independently pinned canonical validator public-key SHA256")
	...
	if *genesisTime == "" || *depositChainID != "3151916" || *forkVersion != "0x31519160" {
		return errors.New("identified native fixture time, chain3151916 and fork0x31519160 are required")
	}
	expectedKey, err := hex.DecodeString(*expectedKeyHash)
	if err != nil || len(expectedKey) != 32 {
		return errors.New("explicit public-key SHA256 required")
	}
	...
	if *genesisTime != "" || *depositChainID != "" {
		...
		if specification.Data["GENESIS_FORK_VERSION"] != *forkVersion {
			return errors.New("native genesis fork changed")
		}
	...
	if forkInfo.Data == nil {
		return errors.New("missing native fork")
	}
	if forkInfo.Data.PreviousVersion != *forkVersion || forkInfo.Data.CurrentVersion != *forkVersion {
		return errors.New("unsupported native fork transition")
	}
	...
	if !bytes.Equal(publicKeyHash[:], expectedKey) {
		return errors.New("canonical validator differs from independently pinned identity")
	}
	messageHash := sha256.Sum256(data)
	result := map[string]any{
		"observedAt":                        time.Now().UTC().Format(time.RFC3339Nano),
		"publicMessageSHA256":               hex.EncodeToString(messageHash[:]),
