...
def main():
    ...
    parser.add_argument("--bootstrap-only", action="store_true",
                        help="export only the explicitly selected trusted bootstrap and its committee proofs")
    args = parser.parse_args()
    bootstrap, attested_slots = args.bootstrap_slot, args.attested_slots
    if args.bootstrap_only:
        attested_slots = []
    if (bootstrap < 0 or bootstrap >= 2**53 or len(attested_slots) > 4
            or attested_slots != sorted(set(attested_slots))
            or (attested_slots and (not bootstrap < attested_slots[0] or attested_slots[-1] >= 2**53))):
        raise ValueError("requires a bootstrap and up to four increasing bounded updates")
    ...
    required_finalized = bootstrap if args.allow_unfinalized_updates or not attested_slots else attested_slots[-1] + 1
    ...
    manifest = {
        ...
        "updatesObservedFinalized": bool(attested_slots) and int(finality["data"]["header"]["message"]["slot"]) >= attested_slots[-1] + 1,
        "updates": updates,
        "files": files,
