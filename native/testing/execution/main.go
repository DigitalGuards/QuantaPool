...
type step struct {
	...
	CaptureAllocation bool   `json:"captureAllocation"`
}
...
type runner struct {
	...
	allocations     map[string]fixtureAllocation
}
...
func (r *runner) run(s step) (uint64, error) {
	...
	if s.Op == "fixture-predeploy" {
		// Explicit synthetic genesis allocation from a constructor that actually
		// ran in this native QRVM. Storage values retain QRL's full 64-byte width.
		allocation, ok := r.allocations[strings.TrimPrefix(s.Source, "@")]
		if !ok || len(allocation.code) == 0 || s.ID == "" {
			return 0, fmt.Errorf("missing constructor-executed fixture allocation")
		}
		if _, exists := r.addresses[s.ID]; exists {
			return 0, fmt.Errorf("duplicate fixture allocation id")
		}
		target, err := r.address(s.Target)
		if err != nil || r.config.State.Exist(target) {
			return 0, fmt.Errorf("fixture allocation target must be a new native address")
		}
		r.config.State.SetCode(target, bytes.Clone(allocation.code))
		r.config.State.SetNonce(target, allocation.nonce)
		r.config.State.SetStorage(target, allocation.storage)
		r.config.State.Finalise(true)
		r.addresses[s.ID] = target
		r.abis[s.ID] = r.abis[strings.TrimPrefix(s.Source, "@")]
		return 0, nil
	}
	if s.Op == "block" {
		value, err := parseInteger(s.Value)
		if err != nil || !value.IsUint64() || value.Cmp(r.config.BlockNumber) <= 0 {
			return 0, fmt.Errorf("synthetic execution block must advance within uint64")
		}
		r.config.BlockNumber.Set(value)
		return 0, nil
	}
	...
	if s.Op == "deploy" {
		...
		var constructorTrace *logger.StructLogger
		if s.CaptureAllocation {
			if config.Value.Sign() != 0 {
				return 0, fmt.Errorf("fixture allocation constructor must have zero value")
			}
			constructorTrace = logger.NewStructLogger(&logger.Config{DisableStack: true})
			config.QRVMConfig.Tracer = constructorTrace
		}
		...
		if callErr == nil {
			...
			if constructorTrace != nil {
				storage := make(map[common.Hash]common.StorageValue64)
				for _, entry := range constructorTrace.StructLogs() {
					for key := range entry.Storage {
						storage[key] = r.config.State.GetState(address, key)
					}
				}
				if r.config.State.GetBalance(address).Sign() != 0 {
					return 0, fmt.Errorf("fixture allocation cannot duplicate a funded account")
				}
				if r.allocations == nil {
					r.allocations = make(map[string]fixtureAllocation)
				}
				r.allocations[s.ID] = fixtureAllocation{bytes.Clone(ret), r.config.State.GetNonce(address), storage}
			}
		}
	} else if s.Op == "call" {
...
func main() {
	...
	r.config = runtime.Config{Origin: origin, State: db, Time: input.Time, GasLimit: params.MaxGasLimit,
		BlockNumber: big.NewInt(1), ChainConfig: &params.ChainConfig{ChainID: big.NewInt(3151916)}}
	...
	if *reportFile != "" {
		encoded, err := json.MarshalIndent(map[string]any{
			"steps":    measurements,
			"boundary": "Actual QRVM and intrinsic gas; synthetic execution environment; no transaction fees debited. Any withdrawal-credit operation executes the native credit routine with an explicitly synthetic unauthenticated payload. Any fixture-predeploy operation creates an explicit synthetic genesis allocation from constructor-executed native code and full-width storage; it changes no network state.",
		}, "", "  ")
		if err != nil {
			panic(err)
		}
