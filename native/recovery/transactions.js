// Isolated actors4/5 for the local timeout regression; separate journal and lock.
...
const runtime = path.join(networkRuntime, 'recovery-timeout');
...
async function context() {
  ...
  assert.equal(process.env.QUANTAPOOL_PUBLIC_DEV_ACCOUNT, '4');
  assert.equal(process.env.QUANTAPOOL_PUBLIC_DEV_CHAIN_ID, '3151916');
  execFileSync('python3', ['-c', 'from context import network_context; network_context()'], { cwd: path.join(root, 'native/lifecycle'), stdio: 'pipe' });
  ...
  async function prepare(name, to, data, value = 0n, actor = 4, nonceOverride = null, fixedGas = null) {
  ...
  async function send(name, to, data, value = 0n, actor = 4) {
    return confirm(await prepare(name, to, data, value, actor));
  }
  async function sendBatch(entries, gasLimit = 8500000n) {
    ...
    const firstNonce = firstSaved ? BigInt(firstSaved.entry.nonce) - BigInt(firstSaved.index) : BigInt(await rpc('qrl_getTransactionCount', [account(4).address, 'pending']));
    const pending = [];
    ...
    for (let start = 0; start < entries.length; start += 8) {
      const results = await Promise.allSettled(entries.slice(start, start + 8).map((entry, offset) => {
        ...
        return prepare(entry.name, entry.to, entry.data, 0n, 4, firstNonce + BigInt(start + offset), gasLimit);
      }));
  ...
  return { network, web3, account, rpc, artifact, contract, send, sendBatch, deploy, predict, defaultActor: 4 };
}
