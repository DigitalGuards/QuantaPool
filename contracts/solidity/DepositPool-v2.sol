...
contract DepositPoolV2 {
    ...
    /// @notice ML-DSA-87 signature length (bytes). qrysm enforces exactly
    /// this value in crypto/ml_dsa_87/ml_dsa_87t/signature.go — any other
    /// length will be rejected by the beacon deposit contract.
    uint256 private constant SIGNATURE_LENGTH = 4627;
    ...
    /// @notice Adjustable floor for minDeposit (owner can lower after deployment)
    uint256 public minDepositFloor = 100 ether;
    ...
    constructor() {
        owner = msg.sender;
        minDeposit = 100 ether; // 100 QRL minimum
    ...
    function claimWithdrawal() external nonReentrant returns (uint256 qrlAmount) {
        ...
        if (requestIndex >= totalRequests) revert NoWithdrawalPending();

        // Update index to account for skipped cancelled requests
        nextWithdrawalIndex[msg.sender] = requestIndex;
    ...
    function fundValidator(
        ...
        // Format: 0x00 (1 byte) + 11 zero bytes + contract address (20 bytes) = 32 bytes.
        // The prefix byte 0x00 is `ExecutionAddressWithdrawalPrefixByte` in qrysm's
        // beacon config (config/params/mainnet_config.go:74), i.e. "withdrawals
        // are executed to the 20-byte address in bytes[12:32]". Ethereum's spec
        // uses 0x01 for the same meaning; QRL v2 re-uses 0x00 because there is
    ...
    function setMinDeposit(uint256 _minDeposit) external onlyOwner {
        if (_minDeposit < minDepositFloor) revert BelowMinDepositFloor();
        minDeposit = _minDeposit;
        emit MinDepositUpdated(_minDeposit);
    }

    /**
     * @notice Set the adjustable floor for minDeposit
     * @dev Allows owner to lower the floor post-deployment (e.g., if QRL appreciates)
     * @param _floor New floor value (must be >= ABSOLUTE_MIN_DEPOSIT)
     */
    function setMinDepositFloor(uint256 _floor) external onlyOwner {
        if (_floor < ABSOLUTE_MIN_DEPOSIT) revert BelowAbsoluteMin();
        minDepositFloor = _floor;
        emit MinDepositFloorUpdated(_floor);
    }
     ...
     *      This is only triggered by explicit transfers (e.g. validator exit
     *      proceeds via a regular transaction).
     *
